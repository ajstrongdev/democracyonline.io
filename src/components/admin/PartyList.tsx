  ...
  const filteredParties = useMemo(() => {
    ...
    return parties.filter((p) => {
      return (
        ...
        (p.leader_username || '').toLowerCase().includes(q)
      );
    });
  }, [parties, searchQuery]);
    ...
    <div className="space-y-4">
      ...
      {searchQuery && (
        <p className="text-sm text-muted-foreground">
          Found {filteredParties.length} part{filteredParties.length !== 1 ? 'ies' : 'y'}
        </p>
      )}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <CardDescription className="mt-1 space-y-0.5">
                    {party.description && (
                      <span className="block text-xs line-clamp-2">{party.description}</span>
                    )}
                    <span className="block text-xs text-muted-foreground">
                      Leader: {party.leader_username || 'None'} • Members: {party.member_count}
                    </span>
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {deletingId === party.id || deleteParty.isPending ? '...' : 'Delete'}
                  </Button>
                </div>
              </div>
            </CardHeader>
        ...
        {filteredParties.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">
              {searchQuery ? 'No parties found matching your search.' : 'No parties to display.'}
            </p>
          </div>
        )}
      </div>
            ...
            <AlertDialogDescription asChild>
              <div>
                ...
                <ul className="list-disc list-inside mt-2 space-y-1">
                  <li>Remove all {partyToDelete?.member_count} members from the party</li>
