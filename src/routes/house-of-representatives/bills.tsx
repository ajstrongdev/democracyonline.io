...
export const Route = createFileRoute("/house-of-representatives/bills")({
  loader: async () => {
    const userData = await getCurrentUserInfo();
    const pageData = await houseBillsPageData();
    const { bills, representatives } = pageData;
...
function RouteComponent() {
  ...
  // Check if user can vote (is a Representative)
  useState(() => {
    if (userData?.id) {
      canVoteOnHouseBill({ data: { userId: userData.id } }).then(setCanVote);
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="mb-8">
          ...
          {bills.length === 0 ? (
            <div className="text-muted-foreground">
              No bills currently in the House of Representatives.
            </div>
          ) : (
                  ...
                  <CardFooter className="flex-col items-start gap-3">
                    ...
                    {canVote === null ? (
                      ...
                      <div className="text-red-500 text-sm">
                        You are not a member of the House of Representatives,
                        you cannot vote on this bill.
                      </div>
        ...
        <Card className="mb-4">
          <CardHeader>
            ...
            <p className="text-muted-foreground">
              Only active representatives are shown.
            </p>
          </CardHeader>
          <CardContent>
                    ...
                    <CardContent className="grow">
                      <div className="flex h-full flex-col gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          {rep.partyId ? (
                            <PartyLogo party_id={rep.partyId} size={40} />
                          ) : (
                            <div className="w-10 h-10 mr-2 rounded-full bg-gray-400" />
                          )}
                          <h3
                            className="text-lg font-medium"
                            style={{ color: rep.partyColor || undefined }}
                          >
                            {rep.partyName || "Independent"}
                          </h3>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() =>
                            router.navigate({ to: `/profile/${rep.id}` })
                          }
          ...
          <CardFooter>
            <p className="text-sm text-muted-foreground">
              Members of the House of Representatives review and vote on
              legislation.
            </p>
          </CardFooter>
        </Card>
